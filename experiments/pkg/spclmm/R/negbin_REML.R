negbin_REML <- function(y, x1, x2, efine = NULL, lcovfine = NULL, C, x1lim = NULL, x2lim = NULL, ndx = c(15, 15), bdeg = c(3, 3), pord = c(2, 2), decom = 1, thr = 1e-06, maxit = 100, parold = c(1, 1, 1), etaold = NULL, trace = FALSE, elements = FALSE) {
  ...
  # Compute deviance
  dev <- 2*sum(y*log(ifelse(y == 0, 1, y/mu)) + (y + lam[3])*log((mu + lam[3])/(y + lam[3])))
  ...
  if (elements) {
    ...
    # Compute updated required matrices
    ZtWZ <- crossprod(Zdip, w*Zdip)
    XtWX <- crossprod(Xdip, w*Xdip)
    ZtWX <- crossprod(Zdip, w*Xdip)
    B1 <- spclmm::inv_bblock2(XtWX, t(ZtWX), ZtWX, ZtWZ + Ginv)
    B2 <- spclmm::bblock2(XtWX, t(ZtWX), ZtWX, ZtWZ)
    ed <- spclmm::trprod(B1$S, B2)
    ...
    sd.eta <- sqrt(diag(X %*% tcrossprod(B1$S11, X)) + 2*diag(X %*% tcrossprod(B1$S12, Z)) + diag(Z %*% tcrossprod(B1$S22, Z)))
