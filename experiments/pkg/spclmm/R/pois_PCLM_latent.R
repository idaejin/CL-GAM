...
pois_PCLM_latent <- function(y, x1, x2, efine = NULL, C,
  ...
  B1 <- spclmm::bbasis(x1, x1lim[1], x1lim[2], ndx[1], bdeg[1])$B
  B2 <- spclmm::bbasis(x2, x2lim[1], x2lim[2], ndx[2], bdeg[2])$B
  # Row-wise tensor product at scattered fine locations (Ayma / Currie style)
  B <- spclmm::rten2(B2, B1)
  c1 <- ncol(B1)
  c2 <- ncol(B2)
  ...
  for (i in seq_len(maxit)) {
    ...
    if (isTRUE(getOption("spclmm.use_rcpp", TRUE)) &&
