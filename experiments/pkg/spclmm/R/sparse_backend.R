...
#' @keywords internal
.spclmm_sparse_backend <- function(backend = NULL) {
  if (is.null(backend) || identical(backend, "auto")) {
    backend <- getOption("spclmm.sparse.backend", "Matrix")
  }
...
#'   or \code{"auto"} (option \code{spclmm.sparse.backend}, default Matrix)
...
as_comp_C <- function(C, backend = "auto") {
  backend <- .spclmm_sparse_backend(backend)
